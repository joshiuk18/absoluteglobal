        ...
        <ul className="grid grid-cols-2 gap-6 sm:grid-cols-4 sm:gap-8">
          {landingCategories.map((cat) => (
            <li key={cat.slug} className="flex flex-col items-center text-center">
              ...
              >
                <span
                  className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border-4 border-white bg-gradient-to-br from-slate-200 to-slate-100 text-xs font-medium text-slate-500 shadow-md ring-1 ring-slate-200/80 transition group-hover:ring-[var(--ag-accent)] sm:h-28 sm:w-28 sm:text-sm"
                  aria-hidden
                >
                  Image
                </span>
